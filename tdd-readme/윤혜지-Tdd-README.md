...
## 회고
사실 엄밀히 말하면 **TDD(Test-Driven Development)** 를 완전히 적용했다기보다는, **DTD(Develop → Test Code 작성 → Develop)** 방식에 가까웠던 것 같습니다.
테스트 코드를 먼저 작성하기보다, 구현 아이디어가 떠오르면 일단 개발을 진행하고 그 후에 테스트 코드를 작성하는 흐름으로 진행했습니다. TDD를 완전히 실천하기는 생각보다 쉽지 않았습니다.
구현할 기능의 **시나리오는 머릿속에 명확히 그려지는데**, 이를 **테스트 코드로 어떻게 표현하고 시나리오를 단계적으로 이어갈지** 고민되는 부분이 많았습니다.
테스트가 설계의 일부가 되어야 한다는 점은 이해했지만, 실제로는 개발 흐름을 끊지 않으면서 테스트 중심으로 사고하는 것이 쉽지 않았습니다.
